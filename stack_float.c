...
int main(int argc, char *argv[])
{
	...
	apix = (float *) malloc(Nx * sizeof(float));
	for (f=1; f<argc-1; f++){
		printf("\nWorking on image: %d\n",f);
		...
		for (firstpix[1]=1;firstpix[1]<=Nx;firstpix[1]++){
			...
			if (fits_read_pix(fptr, TFLOAT, firstpix, Nx, NULL, apix, NULL, &status)) break;
			if (status) break;
			for (i=0;i<Nx;i++){
	...
	fits_create_file(&optr, "out_float.fits", &status);
	...
	for (firstpix[1]=1;firstpix[1]<=Nx;firstpix[1]++){
		fits_write_pix(optr, TFLOAT, firstpix, Nx, outimg[firstpix[1]-1], &status);
	}

...
int main(int argc, char *argv[])
{
	...
	for (f=1; f<argc-1; f++){
		...
		for (firstpix[1]=1;firstpix[1]<=Nx;firstpix[1]++){
			...
			if (fits_read_pix(fptr, TINT, firstpix, Nx, NULL, apix, NULL, &status)) break;
			if (status) break;
			for (i=0;i<Nx;i++){
	...
	for (firstpix[1]=1;firstpix[1]<=Nx;firstpix[1]++){
		fits_write_pix(optr, TINT, firstpix, Nx, outimg[firstpix[1]-1], &status);
	}
